      ...
      <div className="relative z-10 mx-auto w-full max-w-[1200px] px-5 py-20 sm:px-8 sm:py-28">
        <ScrollReveal>
          <div className="flex items-start justify-between gap-8">
            ...
            <div aria-hidden="true" className="hidden shrink-0 text-right lg:block">
              <ul className="space-y-1 text-[0.65rem] font-[500] uppercase tracking-[0.22em] text-white/45">
                {dor.sideRight.map((l) => (
                  <li key={l}>{l}</li>
                ))}
              </ul>
        ...
        <ScrollReveal stagger className="mt-14 grid gap-6 sm:grid-cols-3">
          {dor.items.map((item, i) => (
            <article
              key={item.title}
              className="group surface-dark relative rounded-2xl p-8 backdrop-blur-sm transition-[transform,border-color,box-shadow] duration-300 hover:-translate-y-1 hover:border-mercury/35 hover:shadow-[0_18px_48px_-24px_rgba(255,90,0,0.55)]"
            >
              <span aria-hidden="true" className="text-3xl font-[800] text-mercury">
                {String(i + 1).padStart(2, "0")}
              </span>
              <h3 className="mt-5 text-lg font-[700] leading-snug text-starlight">
                {item.title}
              </h3>
              <p className="mt-4 leading-relaxed text-silver">{item.body}</p>
              <span
                aria-hidden="true"
        ...
        <ScrollReveal className="mt-16 text-center">
          <p className="font-display text-2xl font-[300] tracking-[-0.01em] text-starlight sm:text-3xl">
            {dor.outro.lead}{" "}
